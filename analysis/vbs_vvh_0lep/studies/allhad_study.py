...
def plot_histograms(histo_dict, output_dir, variables=None, categories=None, systematic="nominal", groups=None,
    ...
    for cat in categories:
        for var in variables:
            ...
            # Apply custom Variable settings from YAML if provided
            custom_logY = logY
            custom_xlabel = None
            custom_extra_text = extra_text or f"Category: {cat}"
            if variables_def and var in variables_def:
                plot_def = variables_def[var].get("plot", {})
                custom_xlabel = plot_def.get("xlabel")
                custom_logY = plot_def.get("logY", logY)
            ...
            if logY:
                plot_name += "_logY"
            #if custom_extraTag: # use for different plotting options (e.g. density vs. normal)
...
def main():
    ...
    parser.add_argument("--variables_yaml", default="../variables.yaml", help="Path to YAML file defining variables {var_name: {plot: {xlabel, logY, ...}}}.")
    ...
    print("==="*10)
     ...
     # Load variables_def from YAML
    variables_def = None
    if args.variables_yaml:
        with open(args.variables_yaml, "r") as f:
            variables_def = yaml.safe_load(f)
    ...
    if args.do_plot:
        plot_histograms(histo_dict, output_dir, variables=use_variables, categories=use_categories, groups=groups, variables_def=variables_def)
